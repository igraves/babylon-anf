    ...
    public static ANF.Expression traverse(Function<ANF.Expression, ANF.Expression> f, ANF.Expression expr) {
        ...
        return switch (expr) {
            case ANF.Let(var name, var term, var expbody) -> {
               var res = let(name, traverseTerm(f, term), traverse(f, expbody));
               yield f.apply(res);
            }
